...
- Check `packages/design_system/lib/` for an existing widget before creating a new one (`AppButton`, `AppTextField`, `AppDropdown`, `AppBanner`, `AppTag`, `AsyncContent`, `AppNetworkImage`, `PriceLabel`, `ErrorView`, `EmptyView`, `AppLoader`, `PaginatedList`, `ConfirmDialog`, `SwipeDeck`). Add new shared widgets there, not in a feature.
